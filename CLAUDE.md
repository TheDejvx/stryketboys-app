...
- **v5 (current)**: same single-call, no-second-LLM-call design, but the per-row `ROW <n> | ...` line must now be emitted **immediately after that row's own analysis, interleaved row-by-row**, instead of batched into one block at the end (see `DESCRIBE_PROMPT`). This structurally prevents the v4 failure mode — there's no longer a separate end-of-response summary that can drift away from what was actually reasoned per row. `parse_decode_analysis()`'s regex doesn't care where in the text the `ROW` lines appear (`finditer` over the whole response), so no parser change was needed for this, only the prompt.
- The raw analysis text is printed to server logs (`railway logs`) on every decode call specifically so a bad read can be diagnosed against what the model actually said, instead of guessing blind — this is exactly how the v4 skipped-row bug above was actually found, instead of continuing to guess at prompt wording blindly. `PYTHONUNBUFFERED=1` is set in the `Dockerfile` — without it, `print()` output never reached Railway's log capture at all.
