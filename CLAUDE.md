...
- **v7 (current)**: back to direct 0/1 color judgment (like v5, whose row-by-row interleaved structure was sound), but now the model is given **two images** in the same call: the full coupon screenshot (for team names/order) plus a deterministically-cropped and ~2.5x upscaled zoom of just the right-hand pills column (`crop_pills_column()` in `app.py` — right ~32% of the image, generous margin, pure Pillow crop/resize, zero model involvement), specifically for judging pill fill color against much bigger, clearer pixels. This targets the actual root cause found in v5 (a fine-detail color perception error on small elements) without depending on any spatial estimate from the model at all.
- The raw analysis text is printed to server logs (`railway logs`) on every decode call specifically so a bad read can be diagnosed against real data instead of guessing blind — this is exactly how the v4/v5/v6 root causes were actually found, not by continuing to guess at prompt wording. `PYTHONUNBUFFERED=1` is set in the `Dockerfile` — without it, `print()` output never reached Railway's log capture at all.
- If v7 still isn't reliable enough, the next step would be per-row cropping (13 small, isolated, further-zoomed images, one per row, sent as a batch) rather than one column-wide crop — more API payload but removes even the "match this zoomed row position to that full-image row position" alignment step the model still has to do in v7.
