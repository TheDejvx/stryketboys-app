...
- **v9 (current, two API calls)**: stopped asking the model to track row identity at all — that's now enforced by code, not inference. **Call 1** (`LOCALIZE_PROMPT`) asks only for a coarse bounding region: roughly where row 1's center and the last row's center are (2 numbers) plus row count — a much lower-precision ask than any previous per-pill/per-row attempt, and `parse_localize()` sanity-checks the result (row count in [10,15], bounds ordered and spanning ≥25% of the image height) against a hardcoded fallback (`FALLBACK_ROW_COUNT/ROW1_Y/LASTROW_Y`) rather than ever trusting obviously-broken numbers, the way v6 didn't. From those 2 anchor points, row spacing is computed by simple linear interpolation and each row's pill area is cropped into its **own fully isolated, ~3x-upscaled image** (`crop_row_band()`) — 13 separate small images, not one long strip. **Call 2** sends the full image plus all N row crops *in a fixed, code-guaranteed order* (image 2 = row 1's crop, image 3 = row 2's crop, etc.) and the prompt is explicit that the model doesn't need to figure out row correspondence — it's handed to it. Row identity can no longer be lost mid-response because each row is a structurally separate, explicitly-labeled-by-position image, not something to track across one long output.
- The raw analysis text (and the localization call's raw output) is printed to server logs (`railway logs`) on every decode call specifically so a bad read can be diagnosed against real data instead of guessing blind — this is exactly how every one of the v4–v8 root causes above were actually found, not by continuing to guess at prompt wording. `PYTHONUNBUFFERED=1` is set in the `Dockerfile` — without it, `print()` output never reached Railway's log capture at all.
- If v9 still isn't reliable enough: this has now gone through 9 rounds of prompt/pipeline engineering for what is, empirically, a genuine model-capability ceiling (fine-grained visual discrimination across many small repeated elements) rather than a fixable bug — team names/kickoff times have read correctly nearly every attempt, it's specifically the 13×3 near-identical pill grid that's hard. The pragmatic fallback, already built, is the editable decode preview: treat AI decode as a fast first draft the user corrects via the tappable pill UI before saving, not something that needs to be perfect unassisted.
- If v8 still isn't reliable enough, the next step would be per-row cropping (13 small, isolated, further-zoomed images, one per row, sent as a batch) instead of one tall composite strip — more API payload, but each row becomes fully independent with no possibility of any cross-row confusion at all.
