...
- Row photo crops (client-side only, purely a review aid — never sent back to Claude) went through several attempts before landing on a reliable one:
  ...
  - Each `ROW` line reporting its **own** `Y` position instead (still present in the schema: `ROW <n> | <home> - <away> | <kickoff> | Y=<0.00-1.00> | 1=.. X=.. 2=..`, parsed in `parse_decode_analysis()`) — the theory was that team-name text should be more reliable than small pill colors. Still drifted in practice — the model's raw numeric coordinate output isn't reliably grounded in real measurement at all, regardless of how simple the target is.
  - A deterministic, non-AI pixel approach was then attempted server-side: detect the coupon's alternating row background shade (white / light tint) via Pillow (collapse a horizontal strip to a 1px column with a BOX-filter resize, look for periodic color transitions). Tested rigorously against synthetic images (multiple peak-detection, windowed-diff, coarse-resize, and matched-filter-grid-search variants) — none separated the true row-boundary signal from team-name-text noise reliably enough to trust blind, without the real photo to validate against. Abandoned; no `detect_row_bands()` shipped.
  - **Current**: the AI's per-row `y` is kept only as a **starting position**, not the source of truth. The user drags two lines directly on the displayed photo (`calibrate-line-row1`/`calibrate-line-lastrow` in `calibrationDragHtml()`, pointer-event dragging in `initDragHandlers()`) to mark row 1 and the last row; `applyDragCalibration()` then linearly interpolates every row's `y` between those two human-confirmed points and regenerates crops. A human confirming against the real image sidesteps the entire estimation-reliability problem — usually needs little or no dragging since the AI's guess is a reasonable starting point.
  - `computeRowCrops()` in `templates/index.html` crops a fixed ±3% (`ROW_CROP_MARGIN`) vertical margin around each row's `y`, full width, via `<canvas>` + `toDataURL` — no extra server round-trip. Each row's edit fields show that row's own cropped snippet directly underneath (`row.cropUrl`); the full original photo is still available collapsed under a "Visa hela originalbilden" `<details>` at the bottom as a fallback reference. `decodePreview.imageUrl` (an `URL.createObjectURL(file)`) is revoked via `releaseDecodePreviewImage()` on cancel/save to avoid leaking blob URLs — the per-row `cropUrl`s are `data:` URLs from canvas, which need no such cleanup.
