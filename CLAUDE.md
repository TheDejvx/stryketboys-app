...
- **v8 (current)**: `crop_row_numbers_and_pills()` in `app.py` — the zoomed second image is now a composite: each row's printed row NUMBER (far-left of the original) is cropped and pasted directly next to that *same* row's pills (far-right of the original), preserving the original y-coordinate so row N's number and row N's pills land at the same vertical position in the composite. This removes the cross-image row-counting step entirely — the model reads a row's number and pills from the same zoomed image at the same glance, no alignment guesswork against the first image needed. Also: a parsed row count that isn't exactly 13 is now a hard error (told to retry), not a silently-returned incomplete coupon — a missing row should never reach the save step unnoticed.
- The raw analysis text is printed to server logs (`railway logs`) on every decode call specifically so a bad read can be diagnosed against real data instead of guessing blind — this is exactly how every one of the v4–v7 root causes above were actually found, not by continuing to guess at prompt wording. `PYTHONUNBUFFERED=1` is set in the `Dockerfile` — without it, `print()` output never reached Railway's log capture at all.
