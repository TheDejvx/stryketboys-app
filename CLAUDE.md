...
- The single decode call now also asks for two cheap extra numbers (`ROW1_Y`/`LASTROW_Y` — vertical center of the first/last row, sanity-checked with a fallback exactly like v9's abandoned localization call, see `parse_decode_analysis()`) so the frontend can show *per-row* context. `computeRowCrops()` in `templates/index.html` uses those two anchors plus linear interpolation to slice the uploaded photo into one small crop per row entirely client-side (`<canvas>` + `toDataURL`, no extra server round-trip, no extra image sent back to Claude) — each row's edit fields now show that row's own cropped snippet directly underneath (`row.cropUrl`), instead of one full-image reference the user had to scroll back and forth to compare against. The full original photo is still available too, collapsed under a "Visa hela originalbilden" `<details>` at the bottom, in case the per-row crop coordinates drift off on a differently-cropped screenshot. `decodePreview.imageUrl` (an `URL.createObjectURL(file)`) is revoked via `releaseDecodePreviewImage()` on cancel/save to avoid leaking blob URLs — the per-row `cropUrl`s are `data:` URLs from canvas, which need no such cleanup.
