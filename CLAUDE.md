...
- Row photo crops (client-side only, purely a review aid — never sent back to Claude): first tried a 2-point global estimate (`ROW1_Y`/`LASTROW_Y`, linearly interpolated across all 13 rows) — small estimation error compounded across the list and later rows visibly drifted onto the *next* row's photo. Fixed by having each `ROW` line report its **own** `Y` position instead (`ROW <n> | <home> - <away> | <kickoff> | Y=<0.00-1.00> | 1=.. X=.. 2=..`, parsed in `parse_decode_analysis()`) — team-name text is something the model reads reliably (unlike small pill colors), so per-row anchoring sidesteps compounding drift entirely; each row crop no longer depends on any other row's estimate. `computeRowCrops()` in `templates/index.html` crops a fixed ±3% (`ROW_CROP_MARGIN`) vertical margin around each row's own `y`, full width, via `<canvas>` + `toDataURL` — no extra server round-trip. Rows with a missing/out-of-range `y` (rare) fall back to even spacing from whichever neighbors do have one. Each row's edit fields show that row's own cropped snippet directly underneath (`row.cropUrl`); the full original photo is still available collapsed under a "Visa hela originalbilden" `<details>` at the bottom as a fallback reference. `decodePreview.imageUrl` (an `URL.createObjectURL(file)`) is revoked via `releaseDecodePreviewImage()` on cancel/save to avoid leaking blob URLs — the per-row `cropUrl`s are `data:` URLs from canvas, which need no such cleanup.
