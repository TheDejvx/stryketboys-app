...
- **v4 (current)**: a single API call. The model reasons in free text row-by-row as before, but instead of a second LLM call to structure it, the response must end with a strict machine-readable `SUMMARY:` block (`ROW <n> | <home> - <away> | <kickoff> | 1=<0/1> X=<0/1> 2=<0/1>` per row), which `parse_decode_analysis()` in `app.py` parses with a plain regex — no LLM involved in the extraction step at all, so nothing can get lost in transcription. `DESCRIBE_PROMPT` and `ROW_LINE_RE` define the exact contract between the prompt and the parser; if you change one, change the other.
- The raw analysis text is printed to server logs (`railway logs`) on every decode call specifically so a bad read can be diagnosed against what the model actually said, instead of guessing blind.
